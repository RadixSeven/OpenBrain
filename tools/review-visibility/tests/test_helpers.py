...
from models import Config, ScanCache, TagRule, ThoughtMetadata
...
class TestApplyTagRules:
    ...
    def test_multiple_rules(self, sample_tag_rules: list[TagRule]) -> None:
    ...
    def test_empty_visibility(self) -> None:
        rules = [TagRule(if_present="x", remove_tag="y")]
...
class TestCache:
    def test_load_missing_cache(self, tmp_path: Path) -> None:
        ...
        assert result == ScanCache()

    def test_save_and_load(self, tmp_path: Path) -> None:
        cache_path = tmp_path / "cache.json"
        data = ScanCache(
            scanned={"id-1": ThoughtMetadata(visibility=["sfw"])},
    ...
    def test_save_overwrites(self, tmp_path: Path) -> None:
        cache_path = tmp_path / "cache.json"
        save_cache(ScanCache(prompt_template_id="v1"), cache_path)
        save_cache(
            ScanCache(
                scanned={"x": ThoughtMetadata()},
    ...
    def test_cache_file_is_valid_json(self, tmp_path: Path) -> None:
        cache_path = tmp_path / "cache.json"
        save_cache(ScanCache(), cache_path)
